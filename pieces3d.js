...
(function () {
  ...
  const PROFILES = {
    ...
    queen:  [[0,0],[1.02,0],[1.06,0.10],[0.98,0.20],[0.80,0.32],[0.60,0.46],[0.46,0.60],[0.34,0.85],[0.27,1.25],[0.25,1.60],[0.29,1.92],[0.45,2.04],[0.49,2.13],[0.42,2.22],[0.40,2.28],[0.47,2.36],[0.40,2.44],[0.40,2.50],[0.60,2.80],[0.68,3.02],[0.66,3.12],[0.50,3.16],[0,3.18]],
  ...
  function decorations(type, mat) {
    ...
    if (type === "king") {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        const b = new THREE.Mesh(new THREE.SphereGeometry(0.12, 16, 16), mat);
        b.position.set(Math.cos(a) * 0.6, 3.08, Math.sin(a) * 0.6);
        g.add(b);
      }
      const tip = new THREE.Mesh(new THREE.SphereGeometry(0.13, 16, 16), mat); tip.position.y = 3.3; g.add(tip);
    } else if (type === "rook") {
      for (let i = 0; i < 4; i++) {
